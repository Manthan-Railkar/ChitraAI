...
export const PosterSpiral: React.FC<PosterSpiralProps> = ({ posterUrls, className = '' }) => {
  ...
  const buildSpiral = useCallback((canvas: HTMLCanvasElement, container: HTMLDivElement, urls: string[]) => {
    // ── State ──
    let scrollOffset = 0;
    ...
    let originalPositions: { x: number; y: number; z: number; offsetX: number; offsetY: number; offsetZ: number }[] = [];
    ...
    const onWheel = (e: WheelEvent) => {
      ...
      targetVelocity = targetVelocity * inertia.directionSmoothing + acceleration * (1 - inertia.directionSmoothing);
    ...
    const onTouchEnd = (e: TouchEvent) => {
      ...
      if (Math.abs(touchVelocity) > 0.001) {
        targetVelocity = Math.max(-inertia.maxSpeed * 1.5, Math.min(inertia.maxSpeed * 1.5, touchVelocity * 1.2));
      }
      touchVelocity = 0;
    };
    ...
    function animate() {
      ...
      if (!isDragging && !isTouching && Math.abs(currentVelocity) < 0.0001 && Math.abs(touchVelocity) < 0.0001) {
  ...
  useEffect(() => {
    ...
    cleanupRef.current = buildSpiral(canvas, container, posterUrls);
